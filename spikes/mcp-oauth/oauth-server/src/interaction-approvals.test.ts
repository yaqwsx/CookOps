...
test("private approval credential has an exact constant-time representation", () => {
  assert(privateCredentialMatches(SECRET, `Bearer ${SECRET.toString("base64url")}`));
  assert.equal(privateCredentialMatches(SECRET, "Bearer wrong"), false);
  assert.equal(privateCredentialMatches(SECRET, `Bearer ${SECRET.toString("base64url")}=`,), false);
  assert.equal(privateCredentialMatches(SECRET, undefined), false);
});
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const approvals = new InteractionApprovalStore(database, SECRET);
  ...
  async () => {
    ...
    try {
      ...
      const request = {
        ...
        headers: {
          authorization: `Bearer ${SECRET.toString("base64url")}`,
