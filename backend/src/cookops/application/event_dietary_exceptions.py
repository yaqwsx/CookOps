...
async def update_event_dietary_exception(
    ...
    try:
        ...
        name_bytes = b""
    if not name or len(name) > 200 or not name_bytes:
        violations.append(FieldViolation("name", "must_be_nonempty_utf8_name"))
    try:
        note_bytes = note.encode("utf-8", "strict") if isinstance(note, str) else b""
    except UnicodeEncodeError:
        note_bytes = b""
    if note is not None and (
        not isinstance(note, str) or (note and not note_bytes) or len(note_bytes) > 131072
    ):
        violations.append(FieldViolation("note", "must_be_utf8_note"))
    ...
    async with session_factory() as session, session.begin():
        ...
        if retained:
            if (
                retained.actor_user_id != context.actor_user_id
                or retained.command_kind != UPDATE_COMMAND_KIND
                or retained.request_hash != request_hash
            ):
                raise ApplicationServiceError("idempotency_mismatch", retry_same_identity=False)
            if retained.outcome == "rejected":
        ...
        if violations:
            ...
            item = await session.scalar(
                ...
                .where(
                    ...
                    EventDietaryException.organization_id == organization_id,
                )
                .with_for_update()
            )
            existing_associations = (
                await session.scalars(
                    ...
                    .where(
                        EventDietaryExceptionTag.exception_id == command.exception_id,
            ...
            retained_tag_ids = {
                x.dietary_tag_id for x in existing_associations if x.retired_at is None
            }
            tags = (
                await session.scalars(
                    select(DietaryTag)
            ...
            if (
                event is None
                or item is None
                or item.event_id != command.event_id
                or item.retired_at is not None
            ):
                deferred = ApplicationServiceError(
                    "validation_failed",
                    field_violations=(
                        FieldViolation("event_id", "must_belong_to_active_event_or_exception"),
                    ),
                    retry_same_identity=False,
                )
            elif not valid_tags:
                deferred = ApplicationServiceError(
                    ...
                    field_violations=(
                        FieldViolation(
                            "tag_ids", "must_belong_to_active_tag_or_existing_association"
                        ),
                    ),
                    retry_same_identity=False,
                )
                ...
                if "tag_ids" in wins:
                    ...
                    for association in old:
                        if association.dietary_tag_id not in tag_ids:
                            association.retired_at, association.retired_by_user_id = (
                                when,
                    ...
                    for tag_id in tag_ids:
                        if tag_id in {x.dietary_tag_id for x in old} and tag_id not in old_by_tag:
                            ...
                            association = EventDietaryExceptionTag(
                                organization_id=organization_id,
                                exception_id=item.id,
                                dietary_tag_id=tag_id,
                                created_by_user_id=context.actor_user_id,
                    ...
                    for association in changes:
                        association_clock = await session.scalar(
                            select(FieldClock)
                            .where(
                                FieldClock.entity_kind == "event_dietary_exception_tag",
                                FieldClock.entity_id == association.id,
                        ...
                        if association_clock:
                            (
                                association_clock.winning_client_wall_time,
                                association_clock.winning_mutation_id,
                            ) = when, mutation_id
                await session.flush()
                first, last = await _reserve_change_range(
                    session, organization_id, mutation_id, 1 + len(changes)
                )
                ...
                record["tag_ids"] = [
                    str(x)
                    for x in (tag_ids if "tag_ids" in wins else [x.dietary_tag_id for x in old])
                ]
                session.add(
                    OrganizationChange(
                        organization_id=organization_id,
                    ...
                    session.add(
                        OrganizationChange(
                            ...
                            payload={
                                "record_schema_version": 1,
                                "record": {
                                    **_tag_record(association, assoc_clocks[0]),
                                    "retired_at": association.retired_at.isoformat()
                                    if association.retired_at
                                    else None,
                                    "retired_by_user_id": str(association.retired_by_user_id)
                                    if association.retired_by_user_id
                                    else None,
                                },
                            },
                        )
                ...
                result = EventDietaryExceptionUpdateResult(
                    mutation_id, item.id, first, last, False, outcome
                )
        if deferred:
            session.add(
                Mutation(
                    ...
                    target_identities=(
                        [
                            {
                                "entity_kind": "event_dietary_exception",
                                "entity_id": str(command.exception_id),
                            }
                        ]
                        if isinstance(command.exception_id, UUID)
                        else []
                    ),
                    request_hash=request_hash,
                    outcome="rejected",
