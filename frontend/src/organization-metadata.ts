...
function validCurrency(value: unknown): value is string {
  return (
    typeof value === "string" &&
    /^[A-Z]{3}$/.test(value) &&
    supportedCurrencies?.has(value) === true
  );
}
function wins(actionAt: string, mutationId: string, current: unknown): boolean {
  ...
  const currentAt =
    typeof current.actionAt === "string"
      ? current.actionAt
      : current.winning_client_wall_time;
  const currentId =
    typeof current.mutationId === "string"
      ? current.mutationId
      : current.winning_mutation_id;
  if (typeof currentAt !== "string" || typeof currentId !== "string")
    return false;
  const next = timestampNanoseconds(actionAt),
    old = timestampNanoseconds(currentAt);
  return (
    next !== undefined &&
    old !== undefined &&
    (next > old || (next === old && mutationId > currentId))
  );
}
function payload(value: unknown): value is Record<string, unknown> {
  if (
    ...
    Object.keys(value).sort().join("\0") !==
      ["default_currency", "description", "name", "organization_id"].join("\0")
  )
    return false;
  return (
    uuid.test(String(value.organization_id)) &&
    validText(value.name, 200) &&
    (value.description === null || validText(value.description, 10000)) &&
    validCurrency(value.default_currency)
  );
}
function metadata(record: CanonicalRecord): Metadata | undefined {
  const fields = record.fields;
  return validText(fields.name, 200) &&
    (fields.description === null ||
      fields.description === undefined ||
      validText(fields.description, 10000)) &&
    typeof fields.default_currency === "string"
    ? {
        name: fields.name,
        description:
...
export async function queueOrganizationMetadata(
  userId: string,
  ...
  if (
    ...
    (input.description !== null && !validText(input.description, 10000)) ||
    !validCurrency(input.default_currency) ||
    (!navigator.onLine &&
      !(await readOfflineAuthorization(userId, organizationId)))
  )
    return;
  ...
  await localDb.transaction(
    ...
    async () => {
      ...
      await localDb.optimisticOverlays.put({
        ...current,
        fields: { ...current.fields, ...input },
        fieldClocks: {
          ...current.fieldClocks,
          ...Object.fromEntries(
            fields.map((field) => [field, { mutationId: id, actionAt }]),
          ),
        },
        updatedAt: actionAt,
      });
...
export async function replayOrganizationMetadata(
  userId: string,
  organizationId: string,
  ...
  if (
    ...
    command.payload.organization_id !== organizationId
  )
    return;
  const canonical = await localDb.canonicalRecords.get([
    userId,
  ...
  const nextFields = { ...current.fields },
    nextClocks = { ...current.fieldClocks };
  for (const field of fields)
    if (wins(command.actionAt, command.id, current.fieldClocks[field])) {
      nextFields[field] = command.payload[field];
      nextClocks[field] = {
...
export function readOrganizationMetadata(
