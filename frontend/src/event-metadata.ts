...
async function currentEvent(
  ...
  return event?.lifecycle === "active" &&
    event.fields.lifecycle === "active" &&
    event.fields.id === eventId &&
    event.fields.organization_id === organizationId
    ? event
    : undefined;
}
...
function values(
  ...
  includeDates = true,
): Partial<Record<MetadataField, unknown>> | undefined {
  ...
  return {
    ...
    general_note: normalizedNote,
    ...(includeDates
      ? { start_date: normalizedStart, end_date: normalizedEnd }
      : {}),
  };
}

export async function queueEventMetadataUpdate(
  userId: string,
  organizationId: string,
  input: EventMetadataInput,
): Promise<void> {
  ...
  await localDb.transaction(
    ...
    async () => {
      ...
      await appendOutboxCommand({
        ...
        payload: { event_id: input.eventId, ...normalized },
        actionAt,
        createdAt: actionAt,
        state: "pending",
      });
...
export async function replayEventMetadataUpdate(
  userId: string,
  organizationId: string,
  ...
  const normalized = values(
    {
      ...
      location:
        payload.location === null ? "" : String(payload.location ?? "\u0000"),
      budgetAmount: String(payload.budget_amount ?? ""),
      generalNote:
        payload.general_note === null
          ? ""
          : String(payload.general_note ?? "\u0000"),
  ...
  if (
    ...
    (payload.general_note !== null &&
      note(payload.general_note) !== payload.general_note) ||
    payload.name !== normalized.name ||
    payload.budget_amount !== normalized.budget_amount ||
    (modern &&
      (payload.start_date !== normalized.start_date ||
        payload.end_date !== normalized.end_date))
  )
    return;
  await apply(
    userId,
    ...
    command.id,
    command.actionAt,
    modern ? fields : fields.slice(0, 4),
  );
}
