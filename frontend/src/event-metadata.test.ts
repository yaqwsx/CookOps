...
import { queueEventMetadataUpdate, replayEventMetadataUpdate } from "./event-metadata";
...
describe("event metadata offline command", () => {
  ...
  it("queues all metadata fields into one optimistic command", async () => {
    ...
    await expect(localDb.optimisticOverlays.get([userId, organizationId, "event", eventId])).resolves.toMatchObject({
      fields: { name: "Summer kitchen", location: "Prague", budget_amount: "25.50", general_note: "Bring pots" },
    });
  });
  ...
  it("does not resurrect a stale fractional-clock replay", async () => {
    ...
    await localDb.canonicalRecords.update([userId, organizationId, "event", eventId], {
      fields: {
        ...canonical.fields,
    ...
    await replayEventMetadataUpdate(userId, organizationId, {
      ...
      payload: { event_id: eventId, name: "Stale", location: null, budget_amount: "10", general_note: null },
    });
    await expect(localDb.optimisticOverlays.get([userId, organizationId, "event", eventId])).resolves.toMatchObject({ fields: { name: "Canonical winner", location: null, budget_amount: "10", general_note: null } });
    await replayEventMetadataUpdate(userId, organizationId, {
      id: "fce17d2f-8365-4b1f-a80b-34d10425d51c",
