...
vi.mock("../local-db", () => ({
  readOrCreateBrowserInstallationId: installation,
}));
...
const input = {
  ...
  clientWallTime: "2026-08-18T12:34:56.789Z",
};

beforeEach(() => {
  installation.mockResolvedValue("cce17d2f-8365-4b1f-a80b-34d10425d51c");
...
describe("recipe copy API", () => {
      ...
      async () =>
        new Response(
          JSON.stringify({
            ...
            source_organization_id: ids.sourceOrganizationId,
            destination_organization_id: ids.destinationOrganizationId,
            source_recipe_id: ids.sourceRecipeId,
            destination_recipe_id: ids.destinationRecipeId,
            source_recipe_version_id: ids.sourceCurrentRecipeVersionId,
  ...
  it.each([
    ...
    [
      "zero destination id",
      { destination_organization_id: "00000000-0000-0000-0000-000000000000" },
    ],
    ...
    await expect(
      copyRecipe("user", ids.destinationOrganizationId, input),
    ).rejects.toThrow("Invalid recipe copy response");
  });
});
