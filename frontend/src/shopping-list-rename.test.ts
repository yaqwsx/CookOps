...
describe("offline shopping-list rename", () => {
  it("renames a newly created offline list and preserves create-before-rename ordering", async () => {
    ...
    await queueShoppingListRename(userId, organizationId, {
      shoppingListId: listId as string,
      name: "  Café  ",
    });
    ...
    await expect(
      ...
      fields: { name: "Café" },
      fieldClocks: { name: { mutationId: commands[1].id } },
    });
  });
  ...
  it("replays the ordered rename over the create overlay", async () => {
    ...
    const rename = (await localDb.outbox.toArray()).find(
      (command) => command.commandType === "shopping_list.rename",
    );
    if (!rename) throw new Error("rename command missing");
    await replayShoppingListRename(userId, organizationId, {
      id: rename.id,
      actionAt: rename.actionAt,
    ...
    ).resolves.toMatchObject({ fields: { name: "Café" } });
  });
  ...
  it("queues the canonical NFC name in the exact persisted shape", async () => {
    ...
    await expect(localDb.outbox.toCollection().first()).resolves.toMatchObject({
      ...
      payload: { shopping_list_id: listId, name: "Café" },
      state: "pending",
    });
  });
  ...
  it("rejects stale, malformed, and archived-event commands", async () => {
    await localDb.canonicalRecords.update(
      [userId, organizationId, "shopping_list", listId],
      {
        fieldClocks: {
          name: {
    ...
    await replayShoppingListRename(userId, organizationId, {
      ...
      payload: { shopping_list_id: listId, name: "Stale" },
    });
    await replayShoppingListRename(userId, organizationId, {
      id: "8ce17d2f-8365-4b1f-a80b-34d10425d51c",
      actionAt: "not-a-time",
      payload: { shopping_list_id: listId, name: "Invalid" },
    });
    ...
    await expect(
      queueShoppingListRename(userId, organizationId, {
        shoppingListId: listId,
        name: "Blocked",
      }),
    ...
    await replayShoppingListRename(userId, organizationId, {
      ...
      actionAt: "2026-08-10T12:00:02.000000Z",
      payload: { shopping_list_id: listId, name: "Blocked" },
    });
    expect(await localDb.optimisticOverlays.count()).toBe(0);
  });
});
