...
describe("OrganizationMemberships role controls", () => {
  ...
  it("does not show success when the membership reload fails", async () => {
    const fetchMock = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (fetchMock.mock.calls.length === 3) return new Response("", { status: 500 });
      return new Response(JSON.stringify(memberships("member")), { status: 200 });
    });
    vi.stubGlobal("fetch", fetchMock);
    render(
      <OrganizationMemberships
    ...
    await userEvent.click(await screen.findByRole("button", { name: "Povýšit na administrátora organizace" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("Změnu členství se nepodařilo provést.");
    expect(screen.queryByText("Role administrátora byla aktualizována.")).toBeNull();
  });
  ...
  it("keeps invitation and removal controls out of system administrator management", async () => {
    ...
    expect(await screen.findByRole("button", { name: "Povýšit na administrátora organizace" })).toBeVisible();
    expect(screen.queryByRole("button", { name: "Pozvat člena" })).toBeNull();
    expect(screen.queryByRole("button", { name: "Odebrat člena" })).toBeNull();
    expect(screen.queryByText("Tato organizace zatím nemá administrátora.")).toBeVisible();
  });

  it("shows both empty membership statuses in system administrator management", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response(JSON.stringify({ memberships: [] }), { status: 200 })),
    );
    render(
      <OrganizationMemberships
        onUnauthenticated={vi.fn()}
    ...
    expect(await screen.findByText("Tato organizace zatím nemá žádné členy.")).toBeVisible();
    expect(screen.getByText("Tato organizace zatím nemá administrátora.")).toBeVisible();
  });

  it("confirms administrator revocation", async () => {
    const confirm = vi.spyOn(window, "confirm").mockReturnValue(false);
    const fetchMock = vi.fn(async () => new Response(JSON.stringify(memberships("organization_admin")), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    render(
      <OrganizationMemberships
        onUnauthenticated={vi.fn()}
    ...
    await userEvent.click(await screen.findByRole("button", { name: "Odebrat administrátorskou roli" }));
