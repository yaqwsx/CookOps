...
export function OrganizationMemberships({
  ...
  const load = useCallback(
    async (clearStatus = true): Promise<boolean> => {
      setError(null);
  ...
  async function changeRole(membership: OrganizationMembership) {
    ...
    if (
      ...
      !window.confirm(
        t("membership.revokeConfirm", { email: membership.invitedEmail }),
      )
    )
      return;
    setPending(true);
  ...
  return (
    ...
    >
      ...
      {organizationName ? (
        <p>
          {t("membership.managingOrganization", { name: organizationName })}
        </p>
      ) : null}
      ...
      {memberships?.length === 0 ? (
        <p role="status">{t("membership.noMembers")}</p>
      ) : null}
      {memberships &&
      ((!memberships.length && systemAdminManagement) ||
        (memberships.length > 0 &&
          !memberships.some(
            (membership) =>
              membership.role === "organization_admin" &&
        ...
        <p role="status">{t("membership.noAdmins")}</p>
      ) : null}
        ...
        {memberships?.map((membership) => (
          <li key={membership.id}>
            ...
            {systemAdmin && membership.state === "active" ? (
              ...
              >
                {membership.role === "member"
                  ? t("membership.assignAdmin")
                  : t("membership.revokeAdmin")}
              </button>
            ) : null}
          </li>
        ))}
