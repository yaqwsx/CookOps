...
import {
  ...
  replayIngredientVersionPublish,
...
describe("offline ingredient version publishing", () => {
  it("queues exact immutable payload and advances only optimistic pointer", async () => {
    const versionId = await queueIngredientVersionPublish(
      userId,
      organizationId,
      {
        ingredientId,
    ...
    ).resolves.toMatchObject({ fields: { id: baseId } });
  });
  ...
  it("fails closed on stale replay and keeps the command available for failure handling", async () => {
    const command = {
      ...
      payload: {
        ingredient_id: ingredientId,
        based_on_version_id: "00000000-0000-0000-0000-000000000001",
    ...
    await expect(
      replayIngredientVersionPublish(userId, organizationId, command),
    ).resolves.toBe(false);
  ...
  it("rejects malformed logical operation IDs before writing", async () => {
    await expect(
      queueIngredientVersionPublish(userId, organizationId, {
        ingredientId,
        basedOnVersionId: baseId,
        name: "Tomatoes",
        canonicalUnitId: unitId,
  ...
  it.each(["retired", "foreign"] as const)(
    "fails closed for a %s selected section during queue and replay",
    async (kind) => {
      await localDb.canonicalRecords.update(
        [userId, organizationId, "store_section", sectionId],
        kind === "retired"
          ? { lifecycle: "retired" }
          : {
              fields: {
                organization_id: "ace17d2f-8365-4b1f-a80b-34d10425d51c",
              },
            },
      );
      const input = {
      ...
      await expect(
        queueIngredientVersionPublish(userId, organizationId, input),
