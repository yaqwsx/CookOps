...
export function useEventPendingSync(
  userId: string,
    ...
    const subscription = liveQuery(async () => {
        ...
        [...lists, ...overlays]
          .filter(
            (record) =>
              ...
              record.entityType === "shopping_list" &&
      ...
      for (const record of archives)
        if (
          ...
          eventLists.add(record.entityId);
        ...
        (result, command) => {
          ...
          const belongs =
            ...
            (typeof payload.shopping_list_id === "string" &&
              eventLists.has(payload.shopping_list_id));
          if (belongs)
            command.state === "pending" ? result.pending++ : result.failed++;
          return result;
        },
        { pending: 0, failed: 0 },
      );
...
export function EventSummary({
  ...
  pendingSync: { pending: number; failed: number };
  eventId?: string;
  organizationId?: string;
  userId?: string;
}) {
  ...
  }>({ canManage: false, canDuplicate: false, snapshotId: null });
  useEffect(() => {
    ...
    const subscription = liveQuery(async () => {
      ...
      return {
        ...capabilities,
        snapshotId:
          event.find((candidate) => candidate.id === eventId)
  ...
  const sync =
    [
      pendingSync.failed &&
        t("planner.pendingSyncFailed", { count: pendingSync.failed }),
      pendingSync.pending &&
        t("planner.pendingSyncCount", { count: pendingSync.pending }),
    ]
      .filter(Boolean)
      .join(" · ") || t("planner.pendingSyncCount", { count: 0 });
  return (
    <header className="event-workspace__summary">
      ...
      {planner.lifecycle === "archived" ? (
        <p className="event-workspace__archived" role="status">
          {t("eventSettings.archivedReadOnly")}
        </p>
      ) : null}
      {eventId && organizationId && userId ? (
        <div className="event-workspace__lifecycle">
          {controls.canManage ? (
            <EventLifecycle
              eventId={eventId}
          ...
          {controls.canDuplicate && planner.lifecycle === "archived" ? (
            <EventDuplicate
              eventId={eventId}
              name={planner.name}
              organizationId={organizationId}
              snapshotId={controls.snapshotId}
              userId={userId}
            />
          ) : null}
        </div>
