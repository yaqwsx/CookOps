...
describe("readIngredientCatalog current prices", () => {
  ...
  beforeEach(() => {
    ...
    readVisibleRecords.mockImplementation(
      async (_user: string, _org: string, type: string) => {
        if (type === "ingredient")
          return [
            record("ingredient", ingredientId, {
              id: ingredientId,
              organization_id: organizationId,
              current_version_id: versionId,
              current_price_estimate_id: selectedPriceId,
            }),
          ];
        if (type === "ingredient_version")
          return [
            record("ingredient_version", versionId, {
              id: versionId,
              organization_id: organizationId,
        ...
        if (type === "ingredient_price_estimate")
          return [
            record("ingredient_price_estimate", priceId, {
              id: priceId,
              organization_id: organizationId,
              ingredient_id: ingredientId,
              state: "available",
  ...
  it.each([
    ...
    "ignores %s while preserving a valid estimate",
            ...
            : type === "ingredient_version"
              ...
              : type === "unit_definition"
                ...
                : type === "organization"
                  ...
                  : type === "ingredient_price_estimate"
                    ? [
                        record("ingredient_price_estimate", priceId, {
                          id: priceId,
                          organization_id: organizationId,
                        ...
                        record("ingredient_price_estimate", selectedPriceId, {
                          id: selectedPriceId,
                          organization_id:
                            override.organization_id ?? organizationId,
                          ingredient_id: override.ingredient_id ?? ingredientId,
  ...
  it("preserves a valid current estimate", async () => {
    const result = await readIngredientCatalog(userId, organizationId);
    expect(result.ingredients[0]?.currentPrice).toEqual({
      amount: "2",
      quantity: "1",
      unitId,
      currency: "EUR",
  ...
  it("keeps an active source ingredient whose current unit is retired", async () => {
    ...
    expect(result.sourceUnits).toEqual([
      expect.objectContaining({ id: unitId, name: "old-g" }),
    ]);
    expect(result.ingredients).toEqual([
      expect.objectContaining({ id: ingredientId, canonicalUnitName: "old-g" }),
    ]);
  });
  ...
  it("orders store sections by position key before name", async () => {
    readVisibleRecords.mockImplementation(
      async (_user: string, _org: string, type: string) =>
        type === "store_section"
          ? [
              record("store_section", "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", {
                id: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
                organization_id: organizationId,
                name: "Zeta",
                position_key: "a",
              }),
              record("store_section", "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", {
    ...
    const result = await readIngredientCatalog(userId, organizationId);
    expect(result.storeSections.map((section) => section.name)).toEqual([
      "Zeta",
      "Alpha",
    ]);
  });
