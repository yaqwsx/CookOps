...
export async function queueEventDayCreate(
  userId: string,
  ...
  await localDb.transaction(
    ...
    async () => {
      ...
      if (event?.lifecycle !== "active" || event.fields.lifecycle !== "active")
        throw new Error("selection");
      const existing = await localDb.canonicalRecords
        .where("[userId+organizationId+entityType]")
        .equals([userId, organizationId, "event_day"])
        .filter(
          (record) =>
            record.fields.event_id === input.eventId &&
            record.fields.calendar_date === input.calendarDate &&
            record.lifecycle === "active",
        )
        .first();
      const pending = await localDb.optimisticOverlays
        .where("[userId+organizationId+entityType]")
        .equals([userId, organizationId, "event_day"])
        .filter(
          (record) =>
      ...
      await localDb.optimisticOverlays.put({
        ...
        fieldClocks: { is_visible: { mutationId: id, actionAt } },
        immutable: false,
        updatedAt: actionAt,
      });
...
export async function replayEventDayCreate(
  userId: string,
  organizationId: string,
  ...
  if (
    ...
    ![command.id, p.event_day_id, p.event_id].every(
      (value) => typeof value === "string" && uuid.test(value),
    ) ||
    typeof p.calendar_date !== "string" ||
    !/^\d{4}-\d{2}-\d{2}$/.test(p.calendar_date) ||
  ...
  await localDb.optimisticOverlays.put({
    ...
    fieldClocks: {
      is_visible: { mutationId: command.id, actionAt: command.actionAt },
    },
    immutable: false,
...
function wins(clock: unknown, mutationId: string, actionAt: string): boolean {
  ...
  const current =
    typeof at === "string" ? timestampMicroseconds(at) : undefined;
  return (
    typeof id === "string" &&
    candidate !== undefined &&
    current !== undefined &&
    (candidate > current || (candidate === current && mutationId > id))
  );
}
...
async function apply(
  userId: string,
  organizationId: string,
  input: { eventDayId: string; eventId: string },
  field: "is_visible" | "note",
  ...
  if (
    ...
    canonicalDay?.lifecycle === "retired"
  )
    return false;
  ...
  if (
    ...
    visibleDay?.lifecycle !== "active" ||
    visibleDay.fields.event_id !== input.eventId ||
    !wins(visibleDay.fieldClocks[field], mutationId, actionAt)
  )
    return false;
  await localDb.optimisticOverlays.put({
    ...visibleDay,
    ...
    fieldClocks: {
      ...visibleDay.fieldClocks,
      [field]: { mutationId, actionAt },
    },
    updatedAt: actionAt,
  });
  return true;
...
export async function queueEventDayVisibility(
  userId: string,
  organizationId: string,
  input: VisibilityInput,
): Promise<void> {
  if (![input.eventDayId, input.eventId].every((value) => uuid.test(value)))
    throw new Error("selection");
  const id = crypto.randomUUID();
  const actionAt = new Date().toISOString();
  await localDb.transaction(
    ...
    async () => {
      ...
      await appendOutboxCommand({
        ...
        userId,
        organizationId,
        commandType: "event_day.visibility",
        payload: {
...
export async function replayEventDayVisibility(
  userId: string,
  organizationId: string,
  command: { id: string; actionAt: string; payload: Record<string, unknown> },
): Promise<void> {
  ...
  if (
    ...
    typeof payload.event_day_id !== "string" ||
    typeof payload.event_id !== "string" ||
    typeof payload.is_visible !== "boolean" ||
    ![command.id, payload.event_day_id, payload.event_id].every((value) =>
      uuid.test(value),
    ) ||
    !Number.isFinite(Date.parse(command.actionAt))
  )
  ...
  await apply(
    userId,
    organizationId,
    { eventDayId: payload.event_day_id, eventId: payload.event_id },
    "is_visible",
    payload.is_visible,
    command.id,
    command.actionAt,
...
export async function queueEventDayNote(
  userId: string,
  organizationId: string,
  input: NoteInput,
): Promise<void> {
  ...
  const note =
    typeof rawNote === "string"
      ? rawNote.normalize("NFC").replace(/\r\n?/g, "\n")
      : rawNote;
  if (
    ![input.eventDayId, input.eventId].every((value) => uuid.test(value)) ||
    !validNote(note)
  )
    throw new Error("selection");
  const id = crypto.randomUUID();
...
export async function replayEventDayNote(
  userId: string,
  organizationId: string,
  command: { id: string; actionAt: string; payload: Record<string, unknown> },
): Promise<void> {
  ...
  if (
    ...
    (typeof payload.note === "string" &&
      (payload.note.normalize("NFC").replace(/\r\n?/g, "\n") !== payload.note ||
        !validNote(payload.note)))
  )
    return;
  await apply(
    userId,
...
async function applyLifecycle(
  ...
  if (
    ...
    !visibleDay ||
    visibleDay.fields.event_id !== input.eventId ||
    visibleDay.lifecycle !==
      (input.operation === "retire" ? "active" : "retired") ||
    !wins(visibleDay.fieldClocks.lifecycle, mutationId, actionAt)
  )
    return false;
  await localDb.optimisticOverlays.put({
    ...visibleDay,
...
export async function queueEventDayLifecycle(
  userId: string,
  organizationId: string,
  input: LifecycleInput,
): Promise<void> {
  ...
  await localDb.transaction(
    ...
    localDb.outbox,
    async () => {
      if (!(await applyLifecycle(userId, organizationId, input, id, actionAt)))
        throw new Error("selection");
      await appendOutboxCommand({
        id,
        userId,
        organizationId,
        commandType: "event_day.lifecycle",
        payload: {
          event_day_id: input.eventDayId,
          event_id: input.eventId,
...
export async function replayEventDayLifecycle(
  userId: string,
  organizationId: string,
  command: { id: string; actionAt: string; payload: Record<string, unknown> },
): Promise<void> {
  ...
  await applyLifecycle(
    ...
    organizationId,
    {
      eventDayId: payload.event_day_id,
