...
it("queues lifecycle intent atomically and does not revive a newer canonical lifecycle", async () => {
  ...
  await queueScheduledRecipeLifecycle(user, organization, {
    eventId: event,
    scheduledRecipeId: scheduled,
    operation: "retire",
  });
  await expect(localDb.outbox.toArray()).resolves.toEqual([
    expect.objectContaining({ commandType: "scheduled_recipe.lifecycle" }),
  ]);
  await expect(
  ...
  await replayScheduledRecipeLifecycle(user, organization, {
  ...
  await expect(
    queueScheduledRecipeLifecycle(user, organization, {
      eventId: event,
      scheduledRecipeId: scheduled,
      operation: "retire",
    }),
...
it("replays a valid command without a clock but fails closed for a malformed clock", async () => {
  ...
  ).resolves.toMatchObject({ fields: { diner_count: 3 } });
  await localDb.optimisticOverlays.clear();
  await localDb.canonicalRecords.put({
    ...record,
    fieldClocks: { attendance: {} },
  });
  await replayScheduledRecipeAttendance(user, organization, command);
  await expect(localDb.optimisticOverlays.count()).resolves.toBe(0);
});
  ...
  await localDb.optimisticOverlays.bulkPut([
    {
      ...records[0],
      lifecycle: "retired",
      fields: { ...records[0].fields, lifecycle: "archived" },
    },
    { ...records[1], lifecycle: "retired" },
  ]);
  await expect(
    queueScheduledRecipeAttendance(user, organization, {
      eventId: event,
      scheduledRecipeId: scheduled,
      dinerCount: 3,
    }),
...
it("fails closed for stale context and archived targets", async () => {
  ...
  await expect(
    queueScheduledRecipeContext(user, organization, {
      eventId: event,
      scheduledRecipeId: scheduled,
  ...
  await replayScheduledRecipeContext(user, organization, {
    ...command,
    actionAt: "2026-08-08T13:00:00.000Z",
  });
  await expect(localDb.outbox.count()).resolves.toBe(0);
