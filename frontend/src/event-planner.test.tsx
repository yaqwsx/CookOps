...
vi.mock("./scheduled-ingredient-override", () => ({
  queueAddedOverride,
  queueClearAddedOverride,
  queueClearReplacementOverride,
}));
...
vi.mock("./archive-cache", () => ({
  ensureArchivedEventCached,
  dietaryTagSeedKeys: new Set(["vegetarian", "vegan", "gluten", "lactose"]),
}));
...
describe("EventPlanner", () => {
  ...
  it("edits active scheduled notes and hides controls for archived records", async () => {
    readEventPlanner.mockResolvedValue({
      ...
      roles: [{ id: ids.role, name: "Dinner", position: "a", custom: false }],
      recipes: [],
      scheduled: [
        {
          id: ids.recipe,
          ...
          catalogUpdateChanges: { added: 0, removed: 0, changed: 0 },
          catalogScaleImpact: { reset: false },
        },
      ],
    ...
    render(
      <EventPlanner
        ...
        userId={ids.organization}
      />,
    );
    const note = await screen.findByRole("textbox", {
      name: "Poznámka receptu",
    });
    await userEvent.setup().clear(note);
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "Uložit poznámku" }));
    expect(queueScheduledRecipeNote).toHaveBeenCalledWith(
      ids.organization,
      ids.organization,
      expect.objectContaining({ note: null }),
    );
    readEventPlanner.mockResolvedValueOnce({
      name: "Event",
      startDate: "2026-08-10",
      endDate: "2026-08-10",
      attendance: 1,
  ...
  it("shows the existing cost summary and only this event's pending changes", async () => {
    ...
    readEventCosts.mockResolvedValue({
      currency: "CZK",
      budget: "1000",
      total: "700",
      expectedShopping: "500",
      actual: "300",
      remaining: "700",
      missingIngredients: [],
      scheduled: new Map(),
    ...
    await localDb.outbox.bulkAdd([
      {
        ...
        payload: { event_id: ids.event },
        actionAt: "2026-08-10T00:00:00Z",
    ...
    await waitFor(() =>
      expect(
        screen.getByText((text) => text.includes("1 neúspěšných změn")),
      ).toBeVisible(),
    );
  });
  ...
  it.each([
    [
      ...
      "10. srpna 2026",
      "11. srpna 2026",
      "10. srpna 2026 až 11. srpna 2026",
    ],
    [
      "en",
      "August 10, 2026",
      "August 11, 2026",
      "August 10, 2026 to August 11, 2026",
    ],
  ] as const)(
    "localizes planner dates in %s",
    async (locale, firstDate, secondDate, range) => {
      ...
      readEventPlanner.mockResolvedValue({
        ...
        hiddenDays: [
          {
            id: "8d8b2b21-c378-4574-9e46-9338c81305ef",
            date: "2026-08-11",
            visible: false,
          },
        ],
        ...
        scheduled: [
          {
            id: "ad8b2b21-c378-4574-9e46-9338c81305ef",
            recipeId: ids.recipe,
            name: "Soup",
            dinerCount: 12,
            dayId: ids.day,
      ...
      expect(
        screen.getByRole("heading", { level: 3, name: firstDate }),
      ).toBeVisible();
      const firstDateOptions = screen.getAllByRole("option", {
        name: firstDate,
      });
      expect(
        firstDateOptions.some(
          (option) => option.getAttribute("value") === ids.day,
        ),
      ).toBe(true);
      expect(
        screen.getByRole("option", { name: secondDate }).getAttribute("value"),
      ).toBe("9d8b2b21-c378-4574-9e46-9338c81305ef");
      const hiddenDays = screen.getByText(
        locale === "cs" ? "Skryté dny" : "Hidden days",
      );
  ...
  it("moves the recipe catalog into an accessible mobile dialog", async () => {
    ...
    expect(
      within(dialog).getByRole("combobox", { name: "Recept" }),
    ).toBeVisible();
    await user.click(
      within(dialog).getByRole("button", { name: "Přidat do plánu" }),
    );
    expect(queueRecipeSchedule).toHaveBeenCalledWith(
      "a6a58bd6-214e-49af-8fae-e5f974bf8e08",
      ids.organization,
  ...
  it("schedules a catalog recipe dropped onto an active meal role", async () => {
    ...
    const source = (await screen.findAllByRole("listitem")).find(
      (item) => item.draggable,
    );
  ...
  it("does not drag retired cards or drop onto an archived planner", async () => {
          ...
          dietaryWarnings: [
            {
              exceptionName: "Alex",
              tagNames: ["vegan"],
              tagDescriptors: [{ id: "dietary-vegan", seedKey: "vegan" }],
    ...
    expect(
      await screen.findByText("Alex: štítky Veganské se týkají surovin Tofu."),
    ).toBeVisible();
    const card = (await screen.findAllByRole("listitem")).find((item) =>
      item.textContent?.includes("Chili"),
    );
    if (!card) throw new Error("retired card missing");
    const transfer = dataTransfer();
    fireEvent.dragStart(card, { dataTransfer: transfer });
    fireEvent.drop(screen.getByRole("region", { name: "Večeře" }), {
  ...
  it("moves a visible scheduled card dropped onto another active slot", async () => {
    ...
    readEventPlanner.mockResolvedValue({
      ...
      lifecycle: "active",
      days: [
        { id: ids.day, date: "2026-08-10", note: null },
        { id: secondDay, date: "2026-08-11", note: null },
      ],
      roles: [
        {
          id: ids.role,
    ...
    expect(
      await screen.findByText("Alex: štítky Vegan se týkají surovin Tofu."),
    ).toBeVisible();
    const transfer = dataTransfer();
    const source = (await screen.findAllByRole("listitem")).find(
      (item) => item.draggable,
    ...
    expect(queueScheduledRecipeMove).toHaveBeenCalledWith(
      ...
      ids.organization,
      {
        ...
        eventId: ids.event,
        eventDayId: secondDay,
        eventMealRoleId: secondRole,
        placement: "end",
      },
    );
  });
  ...
  it("saves a day note through an accessible native textarea", async () => {
    ...
    readEventPlanner.mockResolvedValue({
      ...
      attendance: 12,
      lifecycle: "active",
      days: [{ id: ids.day, date: "2026-08-10", note: "Původní" }],
      roles: [],
      recipes: [],
      scheduled: [],
      ...emptyPlannerCollections,
    ...
    await user.click(
      screen.getByRole("button", { name: "Uložit poznámku dne" }),
    );
    expect(queueEventDayNote).toHaveBeenCalledWith(
      "a6a58bd6-214e-49af-8fae-e5f974bf8e08",
      ids.organization,
      { eventDayId: ids.day, eventId: ids.event, note: "Nová poznámka" },
    );
  });
  ...
  it("adds a custom meal role through a labelled native input", async () => {
    ...
    await user.type(
      await screen.findByLabelText("Název chodu"),
      "Pozdní večeře",
    );
    await user.click(screen.getByRole("button", { name: "Přidat chod" }));
    expect(queueEventMealRoleCreate).toHaveBeenCalledWith(
      "a6a58bd6-214e-49af-8fae-e5f974bf8e08",
      ids.organization,
      { eventId: ids.event, customName: "Pozdní večeře" },
    );
  });
  ...
  it("renames a custom meal role through an accessible native input", async () => {
    ...
    readEventPlanner.mockResolvedValue({
      ...
      days: [],
      ...emptyPlannerCollections,
      roles: [{ id: ids.role, name: "Večeře", position: "a", custom: true }],
      recipes: [],
      scheduled: [],
    });
    pullOrganization.mockResolvedValue(false);
    ...
    expect(queueEventMealRoleName).toHaveBeenCalledWith(
      "a6a58bd6-214e-49af-8fae-e5f974bf8e08",
      ids.organization,
      {
        eventId: ids.event,
        eventMealRoleId: ids.role,
        customName: "Pozdní večeře",
      },
    );
  });
      ...
      scheduled: [
        {
          ...
          catalogScaleImpact: {
            currentUnitId: "a",
            targetUnitId: "b",
            currentUnitName: "porce",
            targetUnitName: "osoba",
            reset: false,
            targetBase: "1",
            suggestedAmount: "12",
          },
  ...
  it("edits scaling through accessible controls", async () => {
    ...
    expect(
      screen.getByText("Doporučené množství: 3 porce"),
    ).toBeInTheDocument();
  ...
  it("shows scaling units and queues the selected catalog-update choice", async () => {
    ...
    readEventPlanner.mockResolvedValue({
      ...
      recipes: [
        {
          id: ids.recipe,
          versionId: "8d8b2b21-c378-4574-9e46-9338c81305ef",
          name: "Chili",
        },
      ],
      scheduled: [
  ...
  it("does not render move controls for an archived planner", async () => {
    ...
    expect(
      screen.getByText("Tato akce je archivovaná a plán je jen pro čtení."),
    ).toBeVisible();
    expect(
      screen.queryByText("Náhled aktualizace katalogu"),
    ).not.toBeInTheDocument();
  ...
  it("adds an active catalog ingredient through the typed local override", async () => {
      ...
      scheduled: [
        {
          ...
          lines: [
            {
              id: "bd8b2b21-c378-4574-9e46-9338c81305ef",
              quantity: "1",
              ingredientId: pinnedIngredientId,
            },
          ],
          localAddedIngredients: [
            {
    ...
    expect(
      within(card).getAllByText("Místní surovina: Paprika: 1"),
    ).toHaveLength(1);
    expect(within(card).getByText("Místní surovina: Paprika: 1")).toBeVisible();
    expect(
      screen.queryByRole("option", { name: "Pinto" }),
    ).not.toBeInTheDocument();
  ...
  it("expands pinned recipe details and keeps archived cards read-only", async () => {
      ...
      scheduled: [
        {
          id: ids.recipe,
          recipeId: ids.recipe,
          recipeVersionId: "old-version",
          recipeVersionName: "Pinned soup",
          recipeDescription: "# Pinned\n<script>bad()</script>",
          scalingUnitName: "portion",
          scaleMode: "manual",
    ...
    expect(
      screen.getByText("Připravená hmotnost: 3 · na strávníka 1.5"),
    ).toBeVisible();
    expect(screen.getByText(/# Pinned/)).toHaveTextContent(
      "<script>bad()</script>",
    );
    expect(
      screen.queryByRole("button", { name: "Upravit škálování" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows and queues reset to catalog for an active replacement", async () => {
          ...
          detailLines: [
            {
              ...
              unitName: "ks",
              localOverride: true,
              replacementOverrideId: "7d8b2b21-c378-4574-9e46-9338c81305ef",
    ...
    const card = (await screen.findAllByRole("listitem")).find((item) =>
      item.textContent?.includes("Salát"),
    );
    if (!card) throw new Error("Scheduled recipe card is missing");
    await user.click(within(card).getByText("Podrobnosti receptu"));
    const reset = within(card).getByRole("button", {
      name: "Vrátit na katalog",
    });
    expect(reset).toBeVisible();
    await user.click(reset);
    expect(queueClearReplacementOverride).toHaveBeenCalledWith(
      "a6a58bd6-214e-49af-8fae-e5f974bf8e08",
      ids.organization,
      {
        eventId: ids.event,
        scheduledRecipeId: ids.recipe,
        targetLineKey: "line",
        overrideId: "7d8b2b21-c378-4574-9e46-9338c81305ef",
      },
    );
  });
  ...
  it("shows and queues removal only for an active added ingredient", async () => {
          ...
          detailLines: [
            {
              ...
              unitName: "ks",
              localOverride: true,
              addedOverrideId: "7d8b2b21-c378-4574-9e46-9338c81305ef",
            ...
            },
            {
              ...
              unitName: "ks",
              localOverride: true,
              replacementOverrideId: "8d8b2b21-c378-4574-9e46-9338c81305ef",
              replacementOverrideActive: true,
            },
          ],
          preparedWeight: null,
    ...
    const remove = within(card).getByRole("button", {
      name: "Odebrat přidanou surovinu",
    });
    expect(remove).toBeVisible();
    expect(
      within(card).queryByRole("button", { name: "Odebrat přidanou surovinu" }),
    ).toHaveAccessibleName("Odebrat přidanou surovinu");
    await user.click(remove);
    expect(queueClearAddedOverride).toHaveBeenCalledWith(
      "a6a58bd6-214e-49af-8fae-e5f974bf8e08",
      ids.organization,
      {
        eventId: ids.event,
    ...
    expect(
      within(card).getAllByRole("button", { name: "Vrátit na katalog" }),
    ).toHaveLength(1);
  });
});
