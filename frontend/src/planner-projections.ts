...
export type PlannedRecipe = {
  ...
  catalogScaleImpact: {
    ...
    targetUnitName: string | undefined;
    reset: boolean;
  ...
  detailLines: {
    ...
    replacementOverrideActive?: true;
    addedOverrideId?: string;
    addedOverrideActive?: true;
  }[];
  ...
  dietaryWarnings?: {
    ...
    tagDescriptors?: { id: string; seedKey?: string; name?: string }[];
  }[];
};
...
export function suggestedScale(
  attendance: number,
  consumption: string,
  ...
  const capacity =
    code === "person"
      ? { value: 1n, scale: 0 }
      : estimated
        ? parseDecimal(estimated)
        : undefined;
  if (!percentage || !capacity || capacity.value <= 0n) return base;
  const numerator =
    BigInt(attendance) * percentage.value * 10n ** BigInt(capacity.scale);
...
function scaledQuantity(
  ...
  return formatTerminatingFraction(
    divide(
      multiply(
        { numerator: source.value, denominator: 10n ** BigInt(source.scale) },
        { numerator: ratio.value, denominator: 10n ** BigInt(ratio.scale) },
      ),
      { numerator: divisor.value, denominator: 10n ** BigInt(divisor.scale) },
    ) as Fraction,
  );
}
...
function archivedWarnings(value: unknown): PlannedRecipe["dietaryWarnings"] {
  ...
  const warnings = value.map((item) => {
    ...
    const tagDescriptors = Array.isArray(descriptors)
      ? descriptors.map(parseArchivedDietaryTagDescriptor)
      : [];
    return typeof fields.exception_name === "string" &&
      fields.exception_name.length > 0 &&
      tagDescriptors.length > 0 &&
      tagDescriptors.every((descriptor) => descriptor !== undefined) &&
      ...
      ? {
          ...
          tagNames: tagDescriptors.map(
            (descriptor) => descriptor?.name ?? descriptor?.seedKey ?? "",
          ),
          ingredientNames: ingredientNames as string[],
          tagDescriptors: tagDescriptors as {
            id: string;
            seedKey?: string;
            name?: string;
          }[],
...
export async function readEventPlanner(
  ...
  const eventRecords = await readVisibleRecords(
    userId,
    organizationId,
    "event",
    true,
  ...
  const archiveRows =
    lifecycle === "archived" && typeof snapshotId === "string"
      ? await localDb.archiveRecords
  ...
  const [
    ...
    exceptionRecords,
    exceptionTagRecords,
    archivedWarningRecords,
  ] = await Promise.all([
    readRecords("event_day", true),
    readRecords("event_meal_role", true),
    readRecords("recipe"),
    readRecords("recipe_version"),
    readRecords("recipe_ingredient_line"),
    readRecords("scheduled_recipe", true),
    readRecords("ingredient", true),
    readRecords("ingredient_version"),
    readRecords("scheduled_ingredient_override"),
    readRecords("unit_definition"),
    readRecords("dietary_tag", true),
    readRecords("event_dietary_exception", true),
    readRecords("event_dietary_exception_tag", true),
    readRecords("resolved_dietary_warning", true),
  ]);
  ...
  const versionRecordsById = new Map(
    versionRecords.map((record) => [record.entityId, record]),
  );
  const recipes = recipeRecords
    ...
    .map((record) => {
      ...
      return {
        ...
        scalingUnitId:
          (versionRecords.find((version) => version.entityId === versionId)
            ?.fields.scaling_unit_id as string | undefined) ?? "",
        scalingUnitName: (() => {
          const unitId = versionRecords.find(
            (version) => version.entityId === versionId,
          )?.fields.scaling_unit_id;
          const unit = unitRecords.find(
            (candidate) => candidate.entityId === unitId,
          );
          return (
            value(unit ?? ({ fields: {} } as CanonicalRecord), "custom_name") ??
            value(unit ?? ({ fields: {} } as CanonicalRecord), "code") ??
            ""
          );
        })(),
        baseScalingAmount:
          (versionRecords.find((version) => version.entityId === versionId)
            ?.fields.base_scaling_amount as string | undefined) ?? "",
        roundSuggestionsUp:
          versionRecords.find((version) => version.entityId === versionId)
            ?.fields.round_suggestions_up === true,
      };
    })
        ...
        (record) =>
          ...
          (() => {
            ...
            const version = versionId
              ? ingredientVersions.get(versionId)
              : undefined;
            return Boolean(
              versionId &&
                version?.immutable === true &&
  ...
  const currentVersionByRecipe = new Map(
    recipeRecords
      .filter(
        (record) =>
          belongsToOrganization(record, organizationId) && hasId(record),
      )
      .map((record) => [record.entityId, value(record, "current_version_id")]),
  );
  const lines = new Map<string, PlannedRecipe["lines"]>();
  const detailLines = new Map<
    PlannedRecipe["recipeVersionId"],
    PlannedRecipe["detailLines"]
  >();
  const rootsById = new Map(
    ingredientRecords
      .filter(
        (record) =>
          hasId(record) && value(record, "organization_id") === organizationId,
      )
      .map((record) => [record.entityId, record]),
  );
  const ingredientUnitNames = new Map(
    unitRecords
      .filter(
        (unit) =>
          hasId(unit) &&
  ...
  const scalingUnitNames = new Map(
    unitRecords
      .filter(
        (unit) =>
          hasId(unit) &&
  ...
  for (const record of lineRecords) {
    ...
    const ingredientId = value(
      ingredientVersions.get(value(record, "ingredient_version_id") ?? "") ??
        record,
      "ingredient_id",
    );
    if (
    ...
    const ingredient = ingredientVersionId
      ? ingredientVersions.get(ingredientVersionId)
      : undefined;
    const ingredientName = ingredient && value(ingredient, "name");
    const unitId =
      value(record, "preferred_display_unit_id") ??
      (ingredient && value(ingredient, "canonical_unit_id"));
    const unitName = unitId ? ingredientUnitNames.get(unitId) : undefined;
    const ingredientRoot =
      ingredient && rootsById.get(value(ingredient, "ingredient_id") ?? "");
    ...
    if (
      ...
      hasId(version) &&
      value(version, "organization_id") === organizationId &&
      ingredient?.immutable === true &&
      hasId(ingredient) &&
      value(ingredient, "organization_id") === organizationId &&
      ingredientRoot &&
      ingredientName &&
      unitName &&
      baseQuantity &&
      parsedBaseQuantity &&
      parsedBaseQuantity.value >= 0n &&
      (includeInPortionWeight === undefined ||
        typeof includeInPortionWeight === "boolean") &&
      lineKey &&
      uuid.test(lineKey)
    ) {
      const note =
        typeof record.fields.note === "string" && record.fields.note
          ? record.fields.note
          : undefined;
      detailLines.set(versionId, [
        ...(detailLines.get(versionId) ?? []),
        {
          id: lineKey,
          name: ingredientName,
          ...
          unitName,
          ...(note ? { note } : {}),
          includeInPortionWeight: includeInPortionWeight !== false,
          ...(value(ingredient, "mass_per_canonical_quantity")
            ? {
                massPerCanonicalQuantity: value(
                  ingredient,
  ...
  const dietaryTagNames = new Map(
    dietaryTagRecords
      .filter(
        (tag) => hasId(tag) && value(tag, "organization_id") === organizationId,
      )
      .map((tag) => [
        tag.entityId,
        value(tag, "name") ?? value(tag, "seed_key"),
      ])
      .filter((entry): entry is [string, string] => Boolean(entry[1])),
  );
  const warningExceptions =
    lifecycle === "active"
      ? exceptionRecords
          .filter(
            (exception) =>
              exception.lifecycle === "active" &&
              hasId(exception) &&
              value(exception, "organization_id") === organizationId &&
              value(exception, "event_id") === eventId,
          )
          .map((exception) => {
            const selected = Array.isArray(exception.fields.tag_ids)
              ? exception.fields.tag_ids.filter(
              ...
              : exceptionTagRecords
                  .filter(
                    (association) =>
                      ...
                      hasId(association) &&
                      value(association, "organization_id") ===
                        organizationId &&
                      value(association, "exception_id") === exception.entityId,
                  )
                  .map((association) => value(association, "dietary_tag_id"))
                  .filter((id): id is string => Boolean(id));
            const tags = [...new Set(selected)].filter((id) =>
              dietaryTagNames.has(id),
            );
            return {
              id: exception.entityId,
              name: value(exception, "name"),
              tags,
            };
          })
          .filter(
            (
              exception,
            ): exception is { id: string; name: string; tags: string[] } =>
              Boolean(exception.name && exception.tags.length),
          )
          .sort(
            (left, right) =>
      ...
      : [];
  const dietaryWarningsByScheduled = new Map<
    string,
    PlannedRecipe["dietaryWarnings"]
  >();
  if (lifecycle === "archived") {
    for (const record of archivedWarningRecords) {
      ...
      if (typeof record.fields.scheduled_recipe_id === "string" && mapped)
        dietaryWarningsByScheduled.set(
          record.fields.scheduled_recipe_id,
          mapped,
        );
    }
  }
  if (warningExceptions.length) {
    ...
    for (const item of scheduledRecords) {
      ...
      const version = recipeVersionId
        ? versionRecords.find(
            (candidate) => candidate.entityId === recipeVersionId,
          )
        : undefined;
      ...
      for (const line of lineRecords.filter(
        (candidate) =>
          value(candidate, "recipe_version_id") === recipeVersionId,
      )) {
        const replacement = overrideRecords.find(
          (candidate) =>
            candidate.lifecycle === "active" &&
            value(candidate, "organization_id") === organizationId &&
            value(candidate, "event_id") === eventId &&
            value(candidate, "scheduled_recipe_id") === item.entityId &&
            candidate.fields.override_kind === "replace" &&
            value(candidate, "target_line_key") === value(line, "line_key"),
        );
        const quantity = replacement
          ? value(replacement, "quantity")
          : value(line, "base_quantity");
        const ingredientVersionId = replacement
          ? value(replacement, "ingredient_version_id")
          : value(line, "ingredient_version_id");
        const parsedQuantity = parseDecimal(quantity);
        const scaled =
          value(line, "scaling_behavior") === "fixed"
            ...
            : Boolean(
                parsedQuantity &&
                  ...
                  selected.value !== 0n &&
                  base.value !== 0n &&
                  parsedQuantity.value !== 0n,
              );
        if (scaled && ingredientVersionId && uuid.test(ingredientVersionId))
          resolved.add(ingredientVersionId);
      }
      for (const override of overrideRecords.filter(
        (candidate) =>
          candidate.lifecycle === "active" &&
          value(candidate, "organization_id") === organizationId &&
          value(candidate, "event_id") === eventId &&
          value(candidate, "scheduled_recipe_id") === item.entityId &&
          candidate.fields.override_kind === "add",
      )) {
        const ingredientVersionId = value(override, "ingredient_version_id");
        if (
          ingredientVersionId &&
          uuid.test(ingredientVersionId) &&
          nonzero(override.fields.quantity)
        )
          resolved.add(ingredientVersionId);
      }
      const warning = warningExceptions
        .map((exception) => {
          ...
          const ingredientNames = [...resolved]
            ...
            .filter((ingredient): ingredient is CanonicalRecord =>
              Boolean(
                ingredient &&
                  ingredient.immutable === true &&
                  value(ingredient, "organization_id") === organizationId,
            ...
            .map((ingredient) => {
              const tags = Array.isArray(ingredient.fields.dietary_tag_ids)
                ? ingredient.fields.dietary_tag_ids.filter(
                    (id): id is string =>
                      typeof id === "string" && dietaryTagNames.has(id),
                  )
                : [];
          ...
          return {
            ...
            tagNames: [...matchingTagIds]
              .map((id) => dietaryTagNames.get(id) as string)
              .sort((a, b) => a.localeCompare(b)),
            tagDescriptors: [...matchingTagIds].map((id) => {
              const tag = dietaryTagRecords.find(
                (record) => record.entityId === id,
              );
              return {
                id,
                seedKey: tag ? value(tag, "seed_key") : undefined,
                name: tag ? value(tag, "name") : undefined,
              };
            }),
            ingredientNames: [...new Set(ingredientNames)].sort((a, b) =>
              a.localeCompare(b),
            ),
          };
        })
      ...
      if (warning.length)
        dietaryWarningsByScheduled.set(item.entityId, warning);
    }
  }
  ...
  for (const record of overrideRecords) {
    ...
    const ingredientRoot =
      version && rootsById.get(value(version, "ingredient_id") ?? "");
    const scheduled = scheduledRecipeId
      ? scheduledRecords.find(
          (candidate) => candidate.entityId === scheduledRecipeId,
        )
      : undefined;
    const ingredientVersionId = value(record, "ingredient_version_id");
    const parsedQuantity = parseDecimal(quantity);
    ...
    if (
      ...
      value(scheduled, "organization_id") === organizationId &&
      value(scheduled, "event_id") === eventId &&
      quantity &&
      parsedQuantity &&
      parsedQuantity.value >= 0n &&
      name &&
      ...
      value(version, "organization_id") === organizationId &&
      ingredientVersionId &&
      value(version, "ingredient_id") &&
      ingredientRoot?.lifecycle === "active" &&
      value(ingredientRoot, "organization_id") === organizationId &&
      value(ingredientRoot, "current_version_id") === ingredientVersionId &&
      ingredientUnitNames.has(value(version, "canonical_unit_id") ?? "") &&
      (includeInPortionWeight === undefined ||
        typeof includeInPortionWeight === "boolean")
    )
      localAddedIngredients.set(scheduled.entityId, [
        ...(localAddedIngredients.get(scheduled.entityId) ?? []),
        {
          id: record.entityId,
          name,
          quantity,
          ingredientId: value(version, "ingredient_id"),
          ingredientVersionId,
          includeInPortionWeight: includeInPortionWeight !== false,
          unitName: ingredientUnitNames.get(
            value(version, "canonical_unit_id") ?? "",
          ) as string,
          massPerCanonicalQuantity: value(
            version,
            "mass_per_canonical_quantity",
          ...
          localOverride: true,
          addedOverrideId: record.entityId,
          addedOverrideActive: true,
        },
      ]);
  }
  const scheduled = (
    scheduledRecords
      ...
      .map((record) => {
        const pinnedVersion = versionRecordsById.get(
          value(record, "recipe_version_id") ?? "",
        );
        const recipeRoot = recipeRecords.find(
          (candidate) => candidate.entityId === value(record, "recipe_id"),
        );
        const pinnedBase =
          pinnedVersion && value(pinnedVersion, "base_scaling_amount");
        const parsedPinnedBase = parseDecimal(pinnedBase);
        const pinnedVersionValid =
          pinnedVersion?.immutable === true &&
          hasId(pinnedVersion) &&
          recipeRoot !== undefined &&
          hasId(recipeRoot) &&
          value(recipeRoot, "organization_id") === organizationId &&
          value(pinnedVersion, "organization_id") === organizationId &&
          value(pinnedVersion, "recipe_id") === value(record, "recipe_id") &&
          typeof value(pinnedVersion, "name") === "string" &&
          Boolean(value(pinnedVersion, "name")) &&
          Boolean(parsedPinnedBase && parsedPinnedBase.value > 0n) &&
          scalingUnitNames.has(value(pinnedVersion, "scaling_unit_id") ?? "");
        const targetRecipe = recipes.find(
          (recipe) => recipe.id === value(record, "recipe_id"),
        );
        const targetVersion = targetRecipe?.versionId
          ? versionRecordsById.get(targetRecipe.versionId)
          : undefined;
        const targetRoot = targetRecipe
          ? recipeRecords.find(
              (candidate) => candidate.entityId === targetRecipe.id,
            )
          : undefined;
        const targetBase = targetVersion
          ? parseDecimal(value(targetVersion, "base_scaling_amount"))
          : undefined;
        const targetVersionValid = Boolean(
          targetRecipe &&
            targetVersion?.immutable === true &&
            hasId(targetVersion) &&
            targetRoot &&
            hasId(targetRoot) &&
            value(targetRoot, "organization_id") === organizationId &&
            value(targetVersion, "organization_id") === organizationId &&
            value(targetVersion, "recipe_id") === targetRecipe.id &&
            value(targetVersion, "name") &&
            targetBase &&
            targetBase.value > 0n &&
            scalingUnitNames.has(value(targetVersion, "scaling_unit_id") ?? ""),
        );
        const catalogUpdateDetails =
          pinnedVersionValid &&
          targetVersionValid &&
          targetRecipe?.versionId &&
          targetRecipe.versionId !== value(record, "recipe_version_id")
            ? (() => {
                const oldLines =
                  detailLines.get(value(record, "recipe_version_id") ?? "") ??
                  [];
                const newLines = detailLines.get(targetRecipe.versionId) ?? [];
                if (
                  (lines.get(value(record, "recipe_version_id") ?? "")
                    ?.length ?? 0) !== oldLines.length ||
                  (lines.get(targetRecipe.versionId)?.length ?? 0) !==
                    newLines.length
                )
                  return undefined;
                const oldById = new Map(
                  oldLines.map((line) => [line.id, line]),
                );
                const newById = new Map(
                  newLines.map((line) => [line.id, line]),
                );
                  ...
                  overrideRecords
                    .filter(
                      (override) =>
                        ...
                        value(override, "organization_id") === organizationId &&
                        value(override, "event_id") === eventId &&
                        value(override, "scheduled_recipe_id") ===
                          record.entityId &&
                        override.fields.override_kind === "replace",
                    )
                    .map((override) => value(override, "target_line_key"))
                    .filter((id): id is string => Boolean(id)),
                );
                return [...new Set([...oldById.keys(), ...newById.keys()])]
                  .map((id) => {
                    ...
                    const localOverride = localIds.has(id)
                      ? { localOverride: true as const }
                      : {};
                    if (!oldLine)
                      return {
                        kind: "added" as const,
                        id,
                        newName: newLine?.name,
                        newQuantity: newLine?.quantity,
                        newUnitName: newLine?.unitName,
                        ...localOverride,
                      };
                    if (!newLine)
                      return {
                        ...
                        oldUnitName: oldLine.unitName,
                        ...localOverride,
                      };
                    const changed =
                      oldLine.name !== newLine.name ||
                      oldLine.quantity !== newLine.quantity ||
                      oldLine.unitName !== newLine.unitName;
                    if (!changed && !localOverride.localOverride)
                      return undefined;
                    return {
                      kind: changed
                        ? ("changed" as const)
            ...
            : undefined;
        const pinnedUnitName = pinnedVersionValid
          ? scalingUnitNames.get(value(pinnedVersion, "scaling_unit_id") ?? "")
          : undefined;
        const pinnedUnit = pinnedVersionValid
          ? unitRecords.find(
              (unit) =>
                unit.entityId === value(pinnedVersion, "scaling_unit_id"),
            )
          : undefined;
        const dinerCount =
          typeof record.fields.diner_count === "number"
            ? record.fields.diner_count
            : attendance;
        const suggestedAmount = pinnedVersionValid
          ? (suggestedScale(
              ...
              String(
                pinnedVersion?.fields.estimated_diners_per_scaling_unit ?? "",
              ),
              pinnedBase,
              pinnedVersion?.fields.round_suggestions_up === true,
            ) ?? null)
          : null;
        const resolvedDetailLines = pinnedVersionValid
          ? (detailLines.get(value(record, "recipe_version_id") ?? "") ?? [])
              .flatMap((line) => {
                const source = lineRecords.find(
                  (candidate) =>
                    value(candidate, "line_key") === line.id &&
                ...
                const replacement =
                  ...
                  overrideRecords.find(
                    (candidate) =>
                      ...
                      value(candidate, "organization_id") === organizationId &&
                      value(candidate, "event_id") === eventId &&
                      value(candidate, "scheduled_recipe_id") ===
                        record.entityId &&
                      candidate.fields.override_kind === "replace" &&
                      value(candidate, "target_line_key") === line.id,
                  );
                const quantity = replacement
                  ? value(replacement, "quantity")
                  : line.quantity;
                ...
                const scaled =
                  quantity && source
                    ? scaledQuantity(
                        quantity,
                        value(record, "selected_scale_amount") ?? "",
                        value(pinnedVersion, "base_scaling_amount") ?? "",
                        value(source, "scaling_behavior") === "fixed",
                      )
                    : undefined;
                const replacementVersionId = replacement
                  ? value(replacement, "ingredient_version_id")
                  : undefined;
                const resolved = replacementVersionId
                  ? ingredientVersions.get(replacementVersionId)
                  : undefined;
                const replacementValid =
                  ...
                  Boolean(
                    resolved &&
                      ...
                      value(resolved, "name") &&
                      ingredientUnitNames.get(
                        value(resolved, "canonical_unit_id") ?? "",
                      ),
                  );
                const resolvedMetadata =
                  replacementVersionId && resolved && replacementValid
                    ? {
                  ...
                  ? [
                      {
                        ...
                        ...(replacement
                          ? {
                              localOverride: true as const,
                              replacementOverrideId: replacement.entityId,
                              replacementOverrideActive: true as const,
                            }
                          : {}),
                        ...(resolvedMetadata
                          ? {
              ...
              .concat(
                (localAddedIngredients.get(record.entityId) ?? []).flatMap(
                  (ingredient) =>
                    parseDecimal(ingredient.quantity)?.value !== 0n
                      ? [ingredient]
                      : [],
                ) as typeof detailLines extends Map<string, infer T>
                  ? T
                  : never,
              )
          : [];
        const toFraction = (value: string): Fraction | undefined => {
        ...
        const weightValues = resolvedDetailLines
          .filter((line) => line.includeInPortionWeight)
          .map((line) => {
            const quantity = toFraction(line.quantity),
              mass = toFraction(line.massPerCanonicalQuantity ?? "");
            return quantity && mass && mass.numerator > 0n
              ? multiply(quantity, mass)
              : undefined;
          });
        const weightTotal =
          weightValues.length ===
            resolvedDetailLines.filter((line) => line.includeInPortionWeight)
              .length &&
          weightValues.every(Boolean) &&
          weightValues.length
            ? weightValues.reduce<Fraction>(
                (sum, value) => ({
                  numerator:
                    sum.numerator * (value as Fraction).denominator +
                    (value as Fraction).numerator * sum.denominator,
                  denominator:
                    sum.denominator * (value as Fraction).denominator,
                }),
                { numerator: 0n, denominator: 1n },
              )
            : undefined;
        const preparedWeight = weightTotal
          ? (formatTerminatingFraction(weightTotal) ?? null)
          : null;
        const perDinerWeight =
          preparedWeight &&
          typeof record.fields.diner_count === "number" &&
          record.fields.diner_count > 0
            ? (formatTerminatingFraction(
                divide(weightTotal as Fraction, {
                  numerator: BigInt(record.fields.diner_count),
                  denominator: 1n,
                }) as Fraction,
              ) ?? null)
            : null;
        return {
          ...
          catalogUpdateAvailable:
            catalogUpdateRecipeVersionIds.has(
              value(record, "recipe_version_id") ?? "",
            ) ||
          ...
          catalogUpdateChanges: (() => {
            const current =
              lines.get(value(record, "recipe_version_id") ?? "") ?? [];
            const targetVersion =
              recipes.find((recipe) => recipe.id === value(record, "recipe_id"))
                ?.versionId ?? "";
            const target = lines.get(targetVersion) ?? [];
            const currentById = new Map(
              current.map((line) => [line.id, line.quantity]),
            );
            const targetById = new Map(
              target.map((line) => [line.id, line.quantity]),
            );
            return {
              added: target.filter((line) => !currentById.has(line.id)).length,
              removed: current.filter((line) => !targetById.has(line.id))
                .length,
              changed: target.filter(
                (line) =>
                  currentById.get(line.id) !== undefined &&
                  currentById.get(line.id) !== line.quantity,
              ).length,
            };
          })(),
          catalogUpdateDetails,
          catalogScaleImpact: (() => {
            const current = versionRecords.find(
              (version) =>
                version.entityId === value(record, "recipe_version_id"),
            );
            const target = recipes.find(
              (recipe) => recipe.id === value(record, "recipe_id"),
            );
            const currentUnitId = current?.fields.scaling_unit_id as
              | string
              | undefined;
            const targetUnitId = target?.scalingUnitId;
            const reset = Boolean(
              currentUnitId && targetUnitId && currentUnitId !== targetUnitId,
            );
            const currentUnit = unitRecords.find(
              (unit) => unit.entityId === currentUnitId,
            );
            const targetUnit = unitRecords.find(
              (unit) => unit.entityId === targetUnitId,
            );
            const currentUnitName = currentUnit
              ? (value(currentUnit, "custom_name") ??
                value(currentUnit, "code"))
              : undefined;
            const code = value(
              targetUnit ?? ({ fields: {} } as CanonicalRecord),
              "code",
            );
            const round = target?.versionId
              ? versionRecords.find(
                  (version) => version.entityId === target.versionId,
                )?.fields.round_suggestions_up === true
              : false;
            const dinerCount =
              typeof record.fields.diner_count === "number"
                ? record.fields.diner_count
                : attendance;
            const suggestedAmount = suggestedScale(
              dinerCount,
              value(record, "consumption_percentage") ?? "",
              code,
              target?.versionId
                ? String(
                    versionRecords.find(
                      (version) => version.entityId === target.versionId,
                    )?.fields.estimated_diners_per_scaling_unit ?? "",
                  )
                : undefined,
              target?.baseScalingAmount,
              round,
            );
            return {
              currentUnitId,
              targetUnitId,
              currentUnitName,
              targetUnitName: target?.scalingUnitName,
              reset,
              targetBase: target?.baseScalingAmount,
              suggestedAmount: reset ? suggestedAmount : undefined,
            };
          })(),
          lines: lines.get(value(record, "recipe_version_id") ?? "") ?? [],
          localAddedIngredients:
            localAddedIngredients.get(record.entityId) ?? [],
          recipeDescription:
            pinnedVersionValid &&
            typeof pinnedVersion?.fields.description === "string"
              ? pinnedVersion.fields.description
              : undefined,
          recipeVersionName: pinnedVersionValid
            ? value(pinnedVersion, "name")
            : undefined,
          scalingUnitName: pinnedUnitName,
          scaleMode:
            record.fields.scale_mode === "manual" ||
            record.fields.scale_mode === "suggested"
              ? record.fields.scale_mode
              : undefined,
          hasLocalOverrides: overrideRecords.some(
            (override) =>
              override.lifecycle === "active" &&
              value(override, "organization_id") === organizationId &&
              value(override, "event_id") === eventId &&
              value(override, "scheduled_recipe_id") === record.entityId,
          ),
          detailLines: resolvedDetailLines,
          preparedWeight,
          perDinerWeight,
          dietaryWarnings:
            dietaryWarningsByScheduled.get(record.entityId) ?? [],
          ...
          retired: record.lifecycle === "retired",
          note:
            typeof record.fields.note === "string" ? record.fields.note : null,
        };
      })
      .filter((item) =>
        Boolean(
