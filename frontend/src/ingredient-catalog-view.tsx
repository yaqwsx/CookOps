...
function IngredientCreateForm({
  ...
  useEffect(() => {
    ...
    setInput((current) => ({
      ...
      defaultStoreSectionId: current.defaultStoreSectionId || storeSections[0]?.id || "",
    }));
    ...
    >
      ...
      <div className="ingredient-create__fields">
        ...
        <label>
          ...
          <select required disabled={!storeSections.length} value={input.defaultStoreSectionId} onChange={(event) => change("defaultStoreSectionId", event.target.value)}>
            {storeSections.map((section) => <option key={section.id} value={section.id}>{section.name}</option>)}
          </select>
        </label>
        <label>
      ...
      {!storeSections.length ? <p role="status">{t("ingredientsCatalog.noStoreSections")}</p> : null}
      {error ? (
        <p role="alert">{t(`ingredientsCatalog.errors.${error}`)}</p>
      ) : null}
      {saved ? <p role="status">{t("ingredientsCatalog.saved")}</p> : null}
      <button disabled={!catalog.units.length || !storeSections.length} type="submit">
