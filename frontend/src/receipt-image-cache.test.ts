...
import {
  cacheReceiptImage,
  readCachedReceiptImage,
} from "./receipt-image-cache";
...
describe("receipt image cache", () => {
  ...
  it("stores only a scoped, content-addressed ready image", async () => {
    ...
    await expect(
      cacheReceiptImage(userId, organizationId, attachment, blob),
    ).resolves.toBe(true);
    await expect(
      readCachedReceiptImage(userId, "bad", attachment),
  ...
  it("fails closed on a poisoned stored blob", async () => {
    ...
    await expect(
      readCachedReceiptImage(userId, organizationId, attachment),
    ).resolves.toBeUndefined();
    await expect(
      localDb.receiptImageCache.get([
        userId,
        organizationId,
        attachmentId,
        contentHash,
