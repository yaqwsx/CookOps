...
const {
  ...
  pull: vi.fn(),
  readSource: vi.fn(),
  readDestination: vi.fn(),
  copy: vi.fn(),
  OrganizationRequestError: class OrganizationRequestError extends Error {
  ...
  RecipeCopyRequestError: class RecipeCopyRequestError extends Error {
    constructor(readonly status: number) {
      super();
    }
  },
...
vi.mock("./api/organizations", () => ({
  getAvailableOrganizations: organizations,
  OrganizationRequestError,
}));
vi.mock("./api/recipe-copy", () => ({
  copyRecipe: copy,
  RecipeCopyRequestError,
}));
...
vi.mock("./recipe-copy-catalog", () => ({
  ...
  matchingIds: (values: { id: string; name: string }[], name: string) =>
    values
      .filter((value) => value.name.toLowerCase() === name.toLowerCase())
      .map((value) => value.id),
  normalized: (value: string) =>
    value.normalize("NFC").trim().toLocaleLowerCase(),
  compatibleUnit: (
    source:
      | { id: string; dimension?: string; baseUnitFactor?: string }
      | undefined,
    destination:
      | { id: string; dimension?: string; baseUnitFactor?: string }
      | undefined,
    ...
    Boolean(
      source &&
        ...
        (source.dimension === "count" || source.dimension === "custom"
          ? source.id === destination.id
          : source.baseUnitFactor &&
            source.baseUnitFactor === destination.baseUnitFactor),
    ),
}));
...
const sourceIngredient = {
  id: "ece17d2f-8365-4b1f-a80b-34d10425d51c",
  versionId: ingredientVersionId,
  name: "Flour",
  canonicalUnitName: "source",
  canonicalUnitId: sourceUnitId,
  massPerCanonicalQuantity: "1",
};
const destinationIngredient = {
  ...sourceIngredient,
  id: "fee17d2f-8365-4b1f-a80b-34d10425d51c",
  versionId: "0ce17d2f-8365-4b1f-a80b-34d10425d51c",
  canonicalUnitId: destinationUnitId,
};
    ...
    scalingUnits: [
      {
        ...
        name: "portion",
        dimension: "count",
        baseUnitFactor: undefined,
      },
    ],
    ingredients: source ? [sourceIngredient] : [destinationIngredient],
    units: source ? [sourceUnit] : [destinationUnit],
    tags: [],
    storeSections: [],
    costs: {},
  };
...
async function openAndSelect() {
  ...
  await user.click(
    screen.getByRole("button", { name: "Copy to another organization" }),
  );
  await user.selectOptions(
    await screen.findByRole("combobox", { name: "Destination organization" }),
...
describe("RecipeCopyPanel", () => {
  beforeEach(async () => {
    ...
    organizations.mockResolvedValue([
      { id: sourceOrganizationId, name: "Source" },
      { id: destinationOrganizationId, name: "Destination" },
    ]);
    pull.mockResolvedValue(true);
    copy.mockResolvedValue({});
    readSource.mockResolvedValue(catalog(true));
    readDestination.mockResolvedValue(catalog(false));
  });
  ...
  it("exposes an accessible heading and rejects missing unit metadata", async () => {
    ...
    await openAndSelect();
    expect(
      screen.getByRole("heading", { name: "Copy to another organization" }),
    ).toBeInTheDocument();
  ...
  it("does not map count units by name when their stable identities differ", async () => {
    ...
    readDestination.mockResolvedValue(
      catalog(false, {
        id: destinationUnitId,
        name: "piece",
        dimension: "count",
  ...
  it("discards the prepared snapshot after a definitive copy error", async () => {
    ...
    await openAndSelect();
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "Copy recipe" })).toBeEnabled(),
    );
    await user.click(screen.getByRole("button", { name: "Copy recipe" }));
    await waitFor(() =>
      expect(
        screen.getByRole("combobox", { name: "Destination organization" }),
      ).toHaveValue(""),
    );
  });
