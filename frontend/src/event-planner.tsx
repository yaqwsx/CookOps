...
function CatalogUpdateChoice({ item, planner, eventId, organizationId, userId }: { item: EventPlannerProjection["scheduled"][number]; planner: EventPlannerProjection; eventId: string; organizationId: string; userId: string }) {
  ...
  return <details><summary>{t("planner.catalogUpdatePreview")}</summary><p>{t("planner.catalogUpdateDiff", item.catalogUpdateChanges)}</p>{details ? <ul>{details.map((line) => <li key={`${line.kind}-${line.id}`}><strong>{t(`planner.catalogUpdateKinds.${line.kind}`)}</strong>: {line.oldName && line.newName && line.oldName !== line.newName ? `${line.oldName} → ${line.newName}` : line.newName ?? line.oldName} · {line.oldQuantity ? `${line.oldQuantity} ${line.oldUnitName ?? ""}` : "—"} → {line.newQuantity ? `${line.newQuantity} ${line.newUnitName ?? ""}` : "—"}{line.localOverride ? ` · ${t("planner.localOverrideMarker")}` : ""}</li>)}</ul> : <p role="status">{t("planner.catalogUpdateUnavailable")}</p>}{scale.reset ? (scale.suggestedAmount ? <p>{t("planner.catalogUpdateScaleReset", { current: scale.currentUnitName ?? "—", target: scale.targetUnitName ?? "—", suggestion: scale.suggestedAmount })}</p> : <p>{t("planner.catalogUpdateScaleDeferred")}</p>) : <p>{t("planner.catalogUpdateScaleKept", { unit: scale.targetUnitName ?? scale.currentUnitName ?? "—" })}</p>}{details ? <><p>{t("planner.catalogUpdateChoice")}</p><button type="button" onClick={() => void queue(true)}>{t("planner.preserveOverrides")}</button><button type="button" onClick={() => void queue(false)}>{t("planner.discardOverrides")}</button></> : null}{error ? <p role="alert">{t("planner.errors.unavailable")}</p> : null}</details>;
}
...
function ResetReplacementOverride({ eventId, organizationId, userId, scheduledRecipeId, line }: { eventId: string; organizationId: string; userId: string; scheduledRecipeId: string; line: NonNullable<EventPlannerProjection["scheduled"][number]["detailLines"]>[number] }) {
  const { t } = useTranslation();
  const [error, setError] = useState(false);
  const inFlight = useRef(false);
  if (!line.replacementOverrideId || !line.replacementOverrideActive) return null;
  async function reset() {
    if (inFlight.current) return;
    inFlight.current = true;
    try {
      await queueClearReplacementOverride(userId, organizationId, { eventId, scheduledRecipeId, targetLineKey: line.id, overrideId: line.replacementOverrideId as string });
      setError(false);
    } catch { setError(true); }
