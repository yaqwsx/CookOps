...
describe("event dietary exception create", () => {
  ...
  it("queues an edit against the visible active exception", async () => {
    ...
    await queueEventDietaryExceptionUpdate(
      userId,
      organizationId,
      eventId,
      exceptionId,
      { name: "New", note: "Note", tagIds: [] },
    );
    expect((await localDb.outbox.toArray())[0].commandType).toBe(
      "event_dietary_exception.update",
    );
    expect(
      (
        await localDb.optimisticOverlays.get([
  ...
  it.each([
    ...
    [],
    {
      exception_id: "00000000-0000-4000-8000-000000000031",
      event_id: eventId,
      name: "New",
      note: null,
    },
    ...
    await expect(
      replayEventDietaryExceptionUpdate(userId, organizationId, {
  ...
  it("keeps a newer microsecond canonical field and rejects archived edits", async () => {
    ...
    await localDb.canonicalRecords.put(
      record(
        "event_dietary_exception",
        exceptionId,
        { event_id: eventId, name: "Newer", retired_at: null },
        "active",
      ),
    );
    await localDb.canonicalRecords.update(
      [userId, organizationId, "event_dietary_exception", exceptionId],
      {
        fieldClocks: {
          name: {
            actionAt: "2999-08-16T10:00:00.000001+00:00",
  ...
  it("retains a retired tag only through an active exception association", async () => {
    ...
    await localDb.canonicalRecords.bulkPut([
      ...
      record(
        "dietary_tag",
        tagId,
        { name: "Retired", retired_at: "2026-01-01" },
        "retired",
      ),
    ]);
    await expect(
      queueEventDietaryExceptionUpdate(
        ...
        eventId,
        exceptionId,
        { name: "Kept", note: null, tagIds: [tagId] },
      ),
    ).resolves.toBeUndefined();
    ...
    await expect(
      queueEventDietaryExceptionUpdate(
        ...
        eventId,
        exceptionId,
        { name: "Rejected", note: null, tagIds: [tagId] },
      ),
    ).rejects.toThrow("tag");
  });
...
describe("event dietary exception lifecycle", () => {
  ...
  it("replays lifecycle while preserving pending visible fields and fails closed on malformed clocks", async () => {
    ...
    const command = {
      ...
      actionAt: "2026-08-07T12:00:00.123456Z",
      createdAt: "2026-08-07T12:00:00.123456Z",
    ...
    await expect(
      replayEventDietaryExceptionLifecycle(userId, organizationId, {
        ...command,
        id: "00000000-0000-4000-8000-000000000006",
        actionAt: "bad",
      }),
    ).rejects.toThrow("clock");
    await replayEventDietaryExceptionLifecycle(userId, organizationId, {
      ...command,
      id: "00000000-0000-4000-8000-000000000007",
      actionAt: "2026-08-07T14:00:00.123456+02:00",
      payload: { ...command.payload, operation: "restore" },
    });
    expect(
      (
        await localDb.optimisticOverlays.get([
    ...
    await localDb.optimisticOverlays.put({
      ...
      fieldClocks: {
        ...overlay.fieldClocks,
        lifecycle: { mutationId: "not-a-uuid", actionAt: command.actionAt },
      },
    });
    await expect(
      replayEventDietaryExceptionLifecycle(userId, organizationId, {
  ...
  it("uses the current overlay lifecycle clock and restores a pending create", async () => {
    ...
    if (!current)
      throw new Error("Expected the canonical dietary exception record");
    await localDb.optimisticOverlays.put({
      ...current,
      lifecycle: "retired",
      fieldClocks: {
        lifecycle: {
          ...
          actionAt: "2026-08-07T14:00:00.000000Z",
        },
      },
    });
    await replayEventDietaryExceptionLifecycle(userId, organizationId, {
    ...
    await localDb.optimisticOverlays.delete([
      userId,
      organizationId,
      "event_dietary_exception",
      exceptionId,
