...
vi.mock("./ingredient-copy-catalog", () => ({
  readIngredientCopyCatalog: readCopyCatalog,
}));
...
const previewWire = {
  ...
  mapping_requirements: [
    { kind: "canonical_unit", source_id: sourceUnitId, seed_key: null },
  ],
};
...
describe("IngredientCopyPanel", () => {
  ...
  beforeEach(async () => {
    ...
    readCopyCatalog.mockImplementation(
    ...
    fetchMock.mockImplementation(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes("ingredient-copy-preview"))
          return new Response(JSON.stringify(previewWire), {
            status: previewStatus,
          });
        if (init?.method === "POST" && url.endsWith("/ingredient-copy"))
          return new Response(JSON.stringify(resultWire), {
            status: copyStatus,
          });
    ...
    readInstallationId.mockResolvedValue(
      "dce17d2f-8365-4b1f-a80b-34d10425d51c",
    );
    readVisibleRecords.mockImplementation(
      async (_userId: string, organizationId: string, entityType: string) => {
        const catalog =
          organizationId === sourceOrganizationId
            ? sourceCatalog
            : destinationCatalog;
        if (entityType === "ingredient")
          return organizationId === sourceOrganizationId
            ? [sourceIngredientRecord()]
            : [];
        if (entityType === "ingredient_version")
          return organizationId === sourceOrganizationId
            ? [sourceVersionRecord()]
            : [];
        if (entityType !== "unit_definition") return [];
        return catalog.units.map((unit) =>
          wireRecord(organizationId, entityType, unit.id, {
      ...
      mappings: [
        {
          ...
          destination_id: destinationUnitId,
        },
      ],
    });
  ...
  it("disables confirmation when a required mapping has no destination candidate", async () => {
    readCopyCatalog.mockImplementation(
      async (_userId: string, organizationId: string) => {
        if (organizationId === destinationOrganizationId)
  ...
  it("uses a generic unavailable state for inaccessible previews", async () => {
    ...
    expect(
      await screen.findByText(
        "Copying is not available for this organization.",
      ),
    ).toBeVisible();
  ...
  it("requires a fresh preview after a definite copy rejection", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Confirm copy" }),
  ...
  it("propagates a compatible count mapping and submits both requirements", async () => {
    ...
    sourceCatalog.units = [
      ...
      {
        id: sourceCountUnitId,
        name: "portion",
        dimension: "count",
        baseUnitFactor: undefined,
      },
      {
        id: sourceCustomUnitId,
        name: "serving",
        dimension: "count",
        baseUnitFactor: undefined,
      },
    ...
    destinationCatalog.units = [
      {
        id: destinationCountUnitId,
        name: "piece",
        dimension: "count",
        baseUnitFactor: undefined,
      },
      {
        id: destinationCountUnitId2,
        name: "item",
        dimension: "count",
        baseUnitFactor: undefined,
      },
    ...
    expect(JSON.parse(String(post?.[1]?.body)).mappings).toEqual([
      {
        kind: "canonical_unit",
        source_id: sourceCountUnitId,
        destination_id: destinationCountUnitId2,
      },
      {
        kind: "canonical_unit",
        source_id: sourceCustomUnitId,
        destination_id: destinationCountUnitId2,
      },
    ]);
  });
  ...
  it("reports an unauthorized organization list generically", async () => {
    ...
    fetchMock.mockImplementation(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        if (
          requestPath(input) === "/api/v1/organizations" &&
          isGet(input, init)
        )
  ...
  it("reports an unauthorized destination refresh generically", async () => {
    ...
    pull
      ...
      .mockRejectedValueOnce(new SyncRequestError(401));
    renderPanel(onUnauthenticated);
    await chooseDestination();
    expect(
      await screen.findByText(
  ...
  it("does not reopen after closing while organizations are still loading", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Copy ingredient" }),
    ).not.toBeInTheDocument();
  });
});
